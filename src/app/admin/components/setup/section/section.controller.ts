...
export class SectionController {
	...
	public actionButtonAddOperation(type, section) {
		...
		this.$location.path('/setup/well/' + this.$routeParams.wellId + '/section/' + section.id + '/operation/').search({
	...
	private unableToMakeCurrent(operation) {
		this.dialogService.showMessageDialog('Unable to make operation #' + operation.nextOperation.name + ' current', 'Error');
	}
