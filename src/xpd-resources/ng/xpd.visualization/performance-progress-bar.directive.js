(function () {
	...
	function performanceProgressBar() {
		return {
			templateUrl: '../xpd-resources/ng/xpd.visualization/performance-progress-bar.template.html',
			restrict: 'AE',
			scope: {
				title: '@',
				expectedValue: '=?',
		...
		function link(scope) {
			...
			function prepareLegendBar() {
				...
				var highPercentage = calcPercentage(scope.highValue, scope.afterLowValue);
				var mediumPercentage = calcPercentage(scope.mediumValue, scope.afterLowValue) - highPercentage;
				var lowPercentage = calcPercentage(scope.lowValue, scope.afterLowValue) - (highPercentage + mediumPercentage);
				var afterLowPercentage = 100 - (highPercentage + mediumPercentage + lowPercentage);
			...
			function prepareCurrentPerformance() {
				var percentage = calcPercentage(scope.currentValue, scope.afterLowValue);
