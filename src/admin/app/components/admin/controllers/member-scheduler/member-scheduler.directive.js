(function () {
	...
	function memberScheduler($compile, ganttService) {
		return {
			restrict: 'A',
			scope: {
				'scheduleList': '=',
		...
		function link(scope, element, attrs) {
			ganttService.gantt().then(function (gantt) {
				...
				gantt.attachEvent('onMouseMove', function (id, event) {
					...
					if (event.path[1].className !== 'gantt_cell')
						var taskId = event.path[1].getAttribute('task_id');
					else
						var taskId = event.path[2].getAttribute('task_id');
					...
					if (taskId != null) {
						...
						var element = document.querySelectorAll('div[task_id=\'' + taskId + '\']');
				...
				function createMonthlyOverlay() {
					...
					var iconSize = 50;
					var dataAreaRect = document.querySelector('.gantt_data_area').getBoundingClientRect();
					...
					var monthlyViewLockIcon = document.createElement('span');
				...
				function renderMonthyOverlay() {
					var monthlyOverlayHeight = document.querySelector('.gantt_task_bg').getBoundingClientRect().height;
				...
				function createIdentificationDiv(taskRowElement, task, event) {
					...
					var dataGridReference = document.querySelector('.gantt_data_area');
				...
				function initTaskCellWidth(){
					var row = document.querySelector('.gantt_task_row'); 

					if(row != null){
						var taskCells = row.querySelectorAll('.gantt_task_cell');
				...
				function renderOddBgColumn() {
					var oldOddBgColumns = document.querySelectorAll('.member-scheduler-odd-bg-column');
					...
					var oddBGColumnReferenceElements = document.querySelectorAll('.gantt_task_row');
				...
				function createOddColumnsDailyView(element) {
					var referenceBoundingRect = element.getBoundingClientRect();

					var oddBGColumn = document.createElement('div');
				...
				scope.scheduleActions.addToGantt = function (type, data) {
					...
					try{
						var newTask = null;
						var placeHolderTask = null;

						if (type == 'function') {
							var functionTaskId = 'f_' + data.id;
