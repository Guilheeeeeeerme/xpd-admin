...
(function(){
	...
	function lessonsLearnedController($scope, $modal, reportsSetupAPIService){
		...
		function getLessonListSuccessCallback(result) {
			...
			var lessons = result.lessons_learned;
			listCategory = result.categories;

			for(var i in lessons){
				var node = insertLessonsInCategory(lessons[i]);
		...
		function insertLessonsInCategory(lesson) {
			...
			var node = listCategory[lesson.lessonLearnedCategory.id];
			var time = (new Date(lesson.endTime) - new Date(lesson.startTime))/1000;
			lesson.time = time;
		...
		function createTreeStructure(node) {
			...
			if (node.parentId) {
				...
				var parentNode = listCategory[node.parentId];
		...
		function setTimeAndLessonLength(nodes) {
			...
			for(var i in nodes){
				...
				var parentNode = listCategory[nodes[i].parentId];
		...
		function createSeriesDataChart(nodes){
			...
			for(var i in nodes){
				...
				if (nodes[i].bestPracticesTime == 0) {
					...
					if (nodes[i].bestPracticesTime == nodes[i].time) {
						...
						var time = nodes[i].time - nodes[i].bestPracticesTime;
			...
			paretoData.sort(function(a, b){
				var totalFailuresA = ((a.lessons && a.lessons.y)?a.lessons.y:0) + ((a.bestPractices && a.bestPractices.y)?a.bestPractices.y:0);
				var totalFailuresB = ((b.lessons && b.lessons.y)?b.lessons.y:0) + ((b.bestPractices && b.bestPractices.y)?b.bestPractices.y:0);
