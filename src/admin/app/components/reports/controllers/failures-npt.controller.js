	...
	function failuresNptController($scope, $modal, reportsSetupAPIService) {
		...
		function getFailureListSuccessCallback(result) {
			...
			for(var i in failures){
				var node = insertFailureInCategory(failures[i]);
		...
		function insertFailureInCategory(failure) {
			...
			var node = categories[failure.category.id];
			var time = (new Date(failure.endTime) - new Date(failure.startTime))/1000;
			failure.time = time;
		...
		function makeSeriesDataChart(nodes) {
        	...
        	for(var i in nodes){
        		...
        		if (nodes[i].nptTime == 0) {
        			...
        			if (nodes[i].nptTime == nodes[i].time) {
        				...
        				var time = nodes[i].time - nodes[i].nptTime;
    		...
    		paretoData.sort(function(a, b){
    			var totalFailuresA = ((a.failures && a.failures.y)?a.failures.y:0) + ((a.npts && a.npts.y)?a.npts.y:0);
    			var totalFailuresB = ((b.failures && b.failures.y)?b.failures.y:0) + ((b.npts && b.npts.y)?b.npts.y:0);
