(function () {
	...
	angular.module('xpd.reports')
		.directive('vreScoreBarChart', vreScoreBarChart);

	vreScoreBarChart.$inject = ['highchartsService'];

	function vreScoreBarChart(highchartsService) {
		return {
			restrict: 'EA',
			scope: {
				chartCategories: '=',
		...
		function link(scope, elem, attr) {
			...
			highchartsService.highcharts().then(function (Highcharts) {
				...
				function createChart(chartCategories, chartData) {
					...
					var series = getDataSeries(chartData);
					...
					return Highcharts.chart(elem[0], highcharts);
				}
				...
				function getDataSeries(data) {
					...
					var positiveVre = data.vre.map(positiveOnly);
					var negativeVre = data.vre.map(negativeOnly);
