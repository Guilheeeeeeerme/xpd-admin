	...
	try {
        ...
        if(trg.className == 'gantt_remove'){
			...
			if (task.type == 'section') {
				...
				gantt.callEvent('onXPDButtonAddMember', [task]);
			} else {
				console.log('[Event] onXPDButtonAddSchedule');
				gantt.callEvent('onXPDButtonAddSchedule', [task]);
			}


		} else if (trg.getAttribute('column_id') == 'add') {

			console.log('[Event] onXPDButtonAddFunction');
			gantt.callEvent('onXPDButtonAddFunction', []);

		} else if (trg.className == 'gantt_tree_content') {

			if (task.type == 'section') {
			    if(task.id != 'f_1'){
				    console.log('[Event] onXPDButtonEditFunction');
				    gantt.callEvent('onXPDButtonEditFunction', [task]);
			    }else{
			        console.log('[Event] Ignoring click due to Team restriction');
			    }
			} else {
				console.log('[Event] onXPDButtonEditMember');
				gantt.callEvent('onXPDButtonEditMember', [task]);
			}
		} else if (trg.className == 'gantt_task_content') {

			console.log('[Event] onXPDButtonEditSchedule');
			gantt.callEvent('onXPDButtonEditSchedule', [task]);
		} else {

			var targetCellClick = gantt.getTask(e.path[1].getAttribute('task_id')); //type
...
gantt._DnD.prototype = {
	dragStart: function (obj, e) {
		...
		var mousemove = gantt.bind(function (e) {
			return this.dragMove(obj, e);
		}, this);
		var scroll = gantt.bind(function (e) {
			return this.dragScroll(obj, e);
		}, this);
...
gantt._render_grid_item = function (item) {
	...
	for (var i = 0; i < columns.length; i++) {
		...
		//XPD ALTERAÇÃO
		else if(col.name == 'remove' && item.parent) {
			value = '<div class=\'gantt_remove\'></div>';
		} else if(col.name == 'remove' && !item.parent) {
			value = '<div></div>';
		} else {
			if (col.template)
	...
	if(item.id == 'f_1'){
		css += ' team_gantt_row team_gantt_row_parent';
	}else if(item.parent == 'f_1'){
		css += ' team_gantt_row team_gantt_row_child';
	}else{
		css += item.$index % 2 === 0 ? '' : ' odd ';
	}
...
gantt._render_bg_line = function (item) {
	...
	var css = 'gantt_task_row';

	if(item.id == 'f_1'){
		css += ' team_gantt_task_row team_gantt_task_row_parent ';
	}else if(item.parent == 'f_1'){
		css += ' team_gantt_task_row team_gantt_task_row_child ';
	}else{
		css += (odd ? ' odd' : '') + (cssTemplate ? ' ' + cssTemplate : '');
	}
...
gantt._task_default_render = function (task) {
	...
	if (task.type === 'section' || task.$no_start)
		return;
