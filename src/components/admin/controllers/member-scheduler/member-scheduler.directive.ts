...
export class MemberSchedulerDirective implements ng.IDirective {
	...
	public link: ng.IDirectiveLinkFn = (
		...
		this.ganttService.gantt().then(function (gantt: any) {
			...
			function renderOddBgColumn() {
				...
				if (oldOddBgColumns.length > 0) {
					// FOR EACH HACK
					[].forEach.call(oldOddBgColumns, function (ele) {
						ele.remove();
					});
				}
			...
			function createOddColumnsDailyView(ele: any) {
			...
			function createOddColumnsMonthlyView(ele: any) {
				[].forEach.call(ele.childNodes, function (cellElement, index) {
					if (index % 2 === 1) {
						const oddBGColumn = document.createElement('div');
						oddBGColumn.className = 'member-scheduler-odd-bg-column';
